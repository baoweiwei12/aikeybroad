...
from app.crud import api_config
...
if config is None:
    print("No enabled api config")
    exit(1)
client = XunFeiAiPPTClient(config.app_id, config.api_secret)  # type: ignore
task_info = client.create_task_from_text("护理不良事件分析")
while True:
    try:
        task_progress = client.get_task_progress(task_info.sid)
        if task_progress.process == 100:
            print(task_progress.pptUrl)
            break
        print(task_progress.process)
        time.sleep(3)
    except XunFeiAiPPTClient.XunFeiAiPPTClientError as e:
        print(e)
        continue
